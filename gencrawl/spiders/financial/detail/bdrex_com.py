...
class BdrexComDetail(FinancialDetailFieldMapSpider):
    ...
    def get_items_or_req(self, response, default_item=None):
        ...
        print(items)
        gross_net_temp=(response.xpath("//div[@class='fund-section']//following::p[1]//small//text()").extract()[0]).split(",")
        for item in items:
    ...
    def fund_manager(self, response):
        ...
        fund_managers=response.xpath("//h2[contains(text(),'Portfolio Managers')]/following::p//strong//text()").extract()
        fund_manager=[]
